...
fn extract_docs(
    ...
    let crate_to_package: HashMap<String, &PackageId> =
        metadata.packages.iter().map(|p| (p.name.as_ref().replace('-', "_"), &p.id)).collect();
    ...
    let resolver = Resolver {
        ...
        path_to_kind: path_to_item_kind_map(krate),
        crate_to_package,
        link_to_latest,
    };
...
fn path_to_item_kind_map(krate: &Crate) -> HashMap<&[String], ItemKind> {
...
impl Resolver<'_> {
    ...
    fn crate_doc_url(&self, name: &str) -> String {
        if matches!(name, "core" | "alloc" | "std") {
            ...
            let link_to_latest = self.link_to_latest && from_workspace.unwrap_or(false);
